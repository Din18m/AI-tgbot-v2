...
async def before(id_teacher, nick_tch, id_student, nick_st, window):
...
async def after(id_teacher, nick_tch, id_student, nick_st, window):
    datetime_obj = window['time']
    date = datetime_obj.strftime("%d.%m")
    time = datetime_obj.strftime("%H:%M")
    await bot.send_message(chat_id=id_teacher, text=NOTIFY_TEXT_after.format(date, time, nick_st), reply_markup=teacher_marks_kb(id_student))
    await bot.send_message(chat_id=id_student, text=NOTIFY_TEXT_after.format(date, time, nick_tch), reply_markup=student_marks_kb(id_teacher))
